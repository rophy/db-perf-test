...
MASTER_CPU_PINNING="${MASTER_CPU_PINNING:-}"
...
echo "VMs: $MASTER_COUNT masters ($MASTER_CPUS CPU / ${MASTER_MEMORY}MB) + $TSERVER_COUNT tservers ($TSERVER_CPUS CPU / ${TSERVER_MEMORY}MB)"
echo "Base image: $VM_BASE_IMG"
if [ -n "$MASTER_CPU_PINNING" ]; then
    echo "Master CPU pinning: $MASTER_CPU_PINNING"
fi
...
# Parse CPU pinning into arrays
IFS=' ' read -ra MASTER_PIN_ARRAY <<< "$MASTER_CPU_PINNING"
IFS=' ' read -ra TSERVER_PIN_ARRAY <<< "$TSERVER_CPU_PINNING"

for i in $(seq 1 "$MASTER_COUNT"); do
    master_cpuset="${MASTER_PIN_ARRAY[$((i-1))]:-}"
    if [ -n "$master_cpuset" ]; then
        echo "  (pinning master-${i} to CPUs: $master_cpuset)"
    fi
    create_vm "${VM_PREFIX}-master-${i}" "$MASTER_CPUS" "$MASTER_MEMORY" "$master_cpuset"
done
...
EOF
for i in $(seq 1 "$MASTER_COUNT"); do
    vm="${VM_PREFIX}-master-${i}"
    echo "MASTER_${i}_IP=${VM_IPS[$vm]}" >> "$VM_DIR/vm-ips.env"
done
for i in $(seq 1 "$TSERVER_COUNT"); do
...
# Auto-generated by setup-vm-virsh.sh
# Compatible with yb-ansible inventory format
[masters]
EOF
for i in $(seq 1 "$MASTER_COUNT"); do
    vm="${VM_PREFIX}-master-${i}"
